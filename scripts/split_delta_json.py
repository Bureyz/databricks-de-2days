...
target_basics = os.path.join(base_dir, "04_delta_lake_basics.ipynb")
target_opt = os.path.join(base_dir, "05_delta_lake_optimization.ipynb")
...
# We'll take the first few cells that look like setup.
# Usually the first cell is title, second is intro.
# Let's look for cells containing "%run" or "spark =" or "import"
setup_cells = []
for i in range(split_index):
    cell = cells[i]
    source_text = "".join(cell.get('source', []))
    if "%run" in source_text or "import" in source_text or "spark.conf.set" in source_text:
...
if os.path.exists(target_basics) and os.path.exists(target_opt):
    ...
    # Optional: Remove original file if desired, but maybe keep for safety for now
